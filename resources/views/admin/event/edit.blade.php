...
@section('heading', 'Edit Event: ' . $event->nama)
  ...
  <div style="background:#1a1d2e;border:1px solid #2a2d3e;border-radius:12px;padding:24px;max-width:560px">
    <form action="{{ route('admin.event.update', $event) }}" method="POST" enctype="multipart/form-data">
      ...
      <div style="display:flex;flex-direction:column;gap:16px">
        ...
        <div>
          <label style="display:block;font-size:12px;font-weight:500;color:#9ca3af;margin-bottom:5px">Event Name <span style="color:#f87171">*</span></label>
        ...
        <div>
          <label style="display:block;font-size:12px;font-weight:500;color:#9ca3af;margin-bottom:5px">Category</label>
        ...
        <div>
          <label style="display:block;font-size:12px;font-weight:500;color:#9ca3af;margin-bottom:5px">Date <span style="color:#f87171">*</span></label>
          <input type="date" name="tanggal" required value="{{ old('tanggal', $event->tanggal->format('Y-m-d')) }}"
            style="width:100%;background:#13151f;border:1px solid #2a2d3e;border-radius:8px;padding:8px 12px;font-size:13px;color:#e2e8f0;outline:none;box-sizing:border-box">
          @error('tanggal')<p style="color:#f87171;font-size:11px;margin-top:4px">{{ $message }}</p>@enderror
        </div>

        <div>
          <label style="display:block;font-size:12px;font-weight:500;color:#9ca3af;margin-bottom:5px">Location</label>
          <input type="text" name="lokasi" value="{{ old('lokasi', $event->lokasi) }}" placeholder="Venue / city..."
            style="width:100%;background:#13151f;border:1px solid #2a2d3e;border-radius:8px;padding:8px 12px;font-size:13px;color:#e2e8f0;outline:none;box-sizing:border-box">
        </div>
        ...
        <div>
          ...
          <textarea name="deskripsi" rows="3"
            style="width:100%;background:#13151f;border:1px solid #2a2d3e;border-radius:8px;padding:8px 12px;font-size:13px;color:#e2e8f0;outline:none;box-sizing:border-box;resize:vertical">{{ old('deskripsi', $event->deskripsi) }}</textarea>
        </div>

        <div>
          <label style="display:block;font-size:12px;font-weight:500;color:#9ca3af;margin-bottom:5px">Event Photo</label>
          @if($event->foto)
          <div style="margin-bottom:8px">
            <img src="{{ asset('storage/' . $event->foto) }}" alt="Event Photo"
              style="width:140px;height:100px;object-fit:cover;border-radius:8px;border:1px solid #2a2d3e">
            <p style="font-size:11px;color:#4b5563;margin-top:4px">Current photo. Upload new to replace.</p>
          </div>
          @endif
          ...
          @error('foto')<p style="color:#f87171;font-size:11px;margin-top:4px">{{ $message }}</p>@enderror
        </div>
