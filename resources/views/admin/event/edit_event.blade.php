        ...
        <form action="{{ route('admin.event.update', $event) }}" method="POST" enctype="multipart/form-data">
            ...
            <div style="display:flex;flex-direction:column;gap:16px">
                    ...
                    <select name="kategori_id"
                        ...
                        @foreach ($kategori as $kat)
                            <option value="{{ $kat->id_kategori }}"
                                {{ old('kategori_id', $event->kategori_id) == $kat->id_kategori ? 'selected' : '' }}>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
                    <div>
                        <label style="display:block;font-size:12px;font-weight:500;color:#9ca3af;margin-bottom:5px">Start
                            Time</label>
                        <input type="time" name="jam_mulai" value="{{ old('jam_mulai', $event->jam_mulai ?? '') }}"
                            style="width:100%;background:#13151f;border:1px solid #2a2d3e;border-radius:8px;padding:8px 12px;font-size:13px;color:#e2e8f0;outline:none;box-sizing:border-box">
                    </div>
                    <div>
                        <label style="display:block;font-size:12px;font-weight:500;color:#9ca3af;margin-bottom:5px">End
                            Time</label>
