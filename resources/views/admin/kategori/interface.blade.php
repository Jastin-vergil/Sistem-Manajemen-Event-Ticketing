        ...
        <div style="background:#1a1d2e;border:1px solid #2a2d3e;border-radius:12px;padding:24px">
            ...
            <form action="{{ route('admin.kategori.store') }}" method="POST">
                ...
                <div style="margin-bottom:12px">
                    <label style="display:block;font-size:12px;font-weight:500;color:#9ca3af;margin-bottom:5px">Category Name
                        <span style="color:#f87171">*</span></label>
                    <input type="text" name="nama_kategori" value="{{ old('nama') }}" placeholder="e.g. Music, Sports..."
                ...
                <tbody>
                    @forelse($kategori as $kat)
                        <tr style="border-top:1px solid #2a2d3e"
                            ...
                            <td style="padding:12px 16px;color:#e2e8f0;font-weight:500">{{ $kat->nama_kategori }}</td>
                            <td style="padding:12px 16px;text-align:center">
                                <span
                                    style="font-size:11px;font-weight:500;padding:3px 10px;border-radius:99px;background:rgba(124,111,247,.15);color:#a78bfa">{{ $kat->events_count }}
                                    events</span>
                            </td>
                            <td style="padding:12px 16px;text-align:center">
                                <div style="display:flex;justify-content:center;gap:8px">
                                    <button onclick="openEdit({{ $kat->id_kategori }}, '{{ $kat->nama_kategori }}')"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" style="text-align:center;padding:48px;color:#4b5563">No categories found.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div id="modal-edit"
        ...
        <div style="background:#1a1d2e;border:1px solid #2a2d3e;border-radius:12px;padding:24px;width:400px">
            ...
            <form id="form-edit" method="POST">
                @csrf @method('PUT')
                <div style="margin-bottom:12px">
                    <label style="display:block;font-size:12px;font-weight:500;color:#9ca3af;margin-bottom:5px">Category
