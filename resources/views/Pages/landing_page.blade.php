            ...
            <div class="container mx-auto px-6 md:px-12 relative z-10 flex flex-col lg:flex-row items-center py-24 xl:py-40"
                ...
                <div class="lg:w-3/5 xl:w-2/5 flex flex-col items-start relative z-10 mb-20 lg:mb-0">
                    ...
                    <h1 class="font-roboto-slab text-5xl md:text-7xl leading-[1.2] mt-4">
                        <span class="bg-clip-text text-transparent bg-gradient-to-l from-white via-indigo-400 to-red-400 block md:inline">
                            Explore
                        </span>
                ...
                {{-- Right: Card Stack --}}
                <div class="lg:w-1/2 w-full flex justify-center lg:justify-end lg:ml-auto relative h-[600px] md:h-[800px] items-center">
                    <div class="relative w-80 md:w-[450px] aspect-[3/4]">
                        ...
                        <template x-for="(event, index) in events" :key="index">
                            <div class="absolute inset-0 transition-all duration-1000 ease-in-out transform-gpu"
                                :class="{
                                    'z-30 opacity-100 scale-100 translate-x-0 translate-y-0 rotate-0 shadow-[20px_20px_60px_rgba(0,0,0,0.9)]': activeStack === index + 1,
                                    'z-20 opacity-80 scale-95 translate-x-12 translate-y-6 rotate-3': activeStack === index + 2,
                                    'z-10 opacity-60 scale-90 translate-x-24 translate-y-12 rotate-6': activeStack === index + 3,
                                    'z-0 opacity-0 scale-80 translate-x-32 translate-y-16 rotate-12': activeStack !== index + 1 && activeStack !== index + 2 && activeStack !== index + 3
                                }">
                               <img :src="'/storage/' + event.foto"
                                :alt="event.name"
